                    ...
                    {dataAboutPage.map((data) => (
                        <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
                            ...
                            <div className="flex flex-col sm:flex-row items-start mb-1 
                                ...
                                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-2xl font-semibold uppercase w-12 h-12 mb-3 sm:mb-0 text-[#ffff] bg-[#2098CC] rounded-full border-2 border-white p-2">
                                    {data.date}
                                </time>
                                <div className="text-xl font-bold text-gray-400">{data.subtitle}</div>
                            </div>
                             <div className="text-slate-400" dangerouslySetInnerHTML={{ __html: data.description }} />
                        </div>
                    ))}
                </div>
            </div>
